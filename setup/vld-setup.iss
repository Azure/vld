...
DefaultDirName={reg:HKLM\{#MyAppRegKey},InstallPath|{autopf}\{#MyAppName}}
...
procedure UpdatePaths(var dirList: string; path32: string; path64: string; pathArm64: string);
...
begin
  ...
  begin
    ...
    else if map[i].Key = 'ARM64' then map[i].Value := UpdatePath(map[i].Value, pathArm64);
  end;
  dirList := Merge(map, '|');
  Log(dirList);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      UpdatePaths(AdditionalIncludeDirectories, ExpandConstant('{app}\include'), ExpandConstant('{app}\include'), ExpandConstant('{app}\include'));
      UpdatePaths(AdditionalLibraryDirectories, ExpandConstant('{app}\lib\Win32'), ExpandConstant('{app}\lib\Win64'), ExpandConstant('{app}\lib\ARM64'));
